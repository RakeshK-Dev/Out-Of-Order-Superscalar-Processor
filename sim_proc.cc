...
void Retire(int width, int iq_size, int rob_size)
{
	...
	while ((i < width))
	{
		...
		if (ROB.rob[ROB.head_idx].ds_rdy)		
		{
			...
			for (int k = 0; k < RMT.RMT_Size; k++)							
			{
				if (RMT.rmt[k].ROB_t == ROB.head_idx)
				{
					RMT.rmt[k].ROB_t = 0;
					RMT.rmt[k].valid = false;
				}
			}
...
void Writeback(int width, int iq_size, int rob_size)
{
	...
	if (WB_reg.reg.size() != 0)
	{
		for (unsigned int i = 0; i < WB_reg.reg.size(); i++)
		{
			...
			WB_reg.reg[i].WB_dur = 1; //WB_reg.reg[i].RT_en - WB_reg.reg[i].WB_en;
		...
		while (check != 0)
		{
			...
			for (unsigned int i = 0; i < EX_reg.reg.size(); i++)
			{
				...
				{	
					...
					EX_reg.reg[i].EX_dur = EX_reg.reg[i].EX_cnt; //EX_reg.reg[i].WB_en - EX_reg.reg[i].EX_en;	
